...
<body class="bg-gray-50 min-h-screen flex">
 ...
 <form action="{{ route('admin.menu.update', $menu->id_menu) }}" method="POST" enctype="multipart/form-data"
      class="bg-red-700 p-6 rounded-lg border border-gray-200 shadow-sm space-y-4 w-full text-white">
    @csrf
    @method('PUT')

    {{-- Kategori --}}
    <div>
        <label for="nama_kategori" class="block text-white font-medium mb-2">Kategori Menu</label>
        ...
        @error('nama_kategori') <p class="text-white text-sm mt-1">{{ $message }}</p> @enderror
    </div>

    {{-- Nama Menu --}}
    <div>
        <label for="nama_menu" class="block text-white font-medium mb-2">Nama Menu</label>
        <input type="text" name="nama_menu" id="nama_menu"
               value="{{ old('nama_menu', $menu->nama_menu) }}"
               class="w-full border border-white bg-white text-black rounded-md
                      focus:ring-red-300 focus:border-red-300 p-2">
        @error('nama_menu') <p class="text-white text-sm mt-1">{{ $message }}</p> @enderror
    </div>

    {{-- Harga --}}
    <div>
        <label for="harga_menu" class="block text-white font-medium mb-2">Harga Menu</label>
        <input type="number" name="harga_menu" id="harga_menu"
               value="{{ old('harga_menu', $menu->harga_menu) }}"
               class="w-full border border-white bg-white text-black rounded-md
                      focus:ring-red-300 focus:border-red-300 p-2"
               min="1000">
        @error('harga_menu') <p class="text-white text-sm mt-1">{{ $message }}</p> @enderror
    </div>

    {{-- Deskripsi --}}
    <div>
        <label for="deskripsi_menu" class="block text-white font-medium mb-2">Deskripsi Menu</label>
        <textarea name="deskripsi_menu" id="deskripsi_menu" rows="4"
                  class="w-full border border-white bg-white text-black rounded-md
                         focus:ring-red-300 focus:border-red-300 p-2">{{ old('deskripsi_menu', $menu->deskripsi_menu) }}</textarea>
        @error('deskripsi_menu') <p class="text-white text-sm mt-1">{{ $message }}</p> @enderror
    </div>

    {{-- Gambar --}}
    <div>
        <label for="gambar_menu" class="block text-white font-medium mb-2">Gambar Menu</label>

        @if($menu->gambar_menu)
            <img src="{{ asset('storage/'.$menu->gambar_menu) }}"
                 class="mb-2 w-32 h-32 object-cover rounded-md border">
        @endif

        <input type="file" name="gambar_menu" id="gambar_menu" accept="image/*"
               class="w-full border border-white bg-white text-black rounded-md
                      focus:ring-red-300 focus:border-red-300 p-2">
        @error('gambar_menu') <p class="text-white text-sm mt-1">{{ $message }}</p> @enderror
    </div>

    {{-- Status --}}
    <div>
        <label for="status_menu" class="block text-white font-medium mb-2">Status Menu</label>
    ...
    {{-- Stok --}}
    <div>
        <label for="stok_menu" class="block text-white font-medium mb-2">Stok Menu</label>
        <input type="number" name="stok_menu" id="stok_menu"
               value="{{ old('stok_menu', $menu->stok_menu) }}"
               class="w-full border border-white bg-white text-black rounded-md
                      focus:ring-red-300 focus:border-red-300 p-2">
        @error('stok_menu') <p class="text-white text-sm mt-1">{{ $message }}</p> @enderror
    </div>
